...
export class ModManagerSubUi {
    ...
    async whenCreate(Ref: ModSubUiAngularJsModeExportInterface) {
        ...
        // Add local mods to the enabled list (local mods are placed before sideload mods)
        const combinedEnabled = Array.from(new Set([...localAll, ...modListEnabled]));
        ...
        Ref.addComponentModGuiConfig({
            ...
            data: {
                ...
                listDisabled: finalDisabled.map(T => {
                    return {
                        key: T,
                        str: T,
                        selected: false,
